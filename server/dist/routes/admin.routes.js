...
router.post('/approve-teacher', async (req, res) => {
    ...
    if (!teacherId || typeof approved !== 'boolean')
        return res.status(400).json({ error: 'Missing fields' });
    const teacher = await User_1.default.findOneAndUpdate({ _id: teacherId, role: 'teacher' }, { approved }, { new: true }).select('name email approved');
    if (!teacher)
...
router.post('/assign-subject', async (req, res) => {
    ...
    if (!subjectId || !teacherId)
        return res.status(400).json({ error: 'Missing fields' });
    const teacher = await User_1.default.findOne({ _id: teacherId, role: 'teacher' });
    if (!teacher)
        return res.status(404).json({ error: 'Teacher not found' });
    const subject = await Subject_1.default.findByIdAndUpdate(subjectId, { teacherId }, { new: true });
    if (!subject)
        return res.status(404).json({ error: 'Subject not found' });
    await User_1.default.findByIdAndUpdate(teacherId, { $addToSet: { assignedSubjects: subject._id } });
    res.json({ subject });
});
...
router.get('/teacher-stats/:teacherId', async (req, res) => {
    ...
    const allStudents = await User_1.default.find({
        role: 'student',
    ...
    const [totalClasses, classesYear, classesMonth, classesWeek] = await Promise.all([
        Class_1.default.countDocuments({ teacherId }),
        Class_1.default.countDocuments({ teacherId, scheduledDate: { $gte: startOfYear } }),
        Class_1.default.countDocuments({ teacherId, scheduledDate: { $gte: startOfMonth } }),
        Class_1.default.countDocuments({ teacherId, scheduledDate: { $gte: startOfWeek } }),
    ]);
    // 3. Subject Breakdown
    const subjectsData = await Subject_1.default.find({ _id: { $in: teacher.assignedSubjects } }).lean();
...
router.post('/upload-resource', async (req, res) => {
    ...
    const resource = await Promise.resolve().then(() => __importStar(require('../models/Resource'))).then(m => m.default.create({
        title,
        type,
        url: finalUrl,
        fileType,
...
router.post('/delete-resource', async (req, res) => {
    ...
    const Resource = (await Promise.resolve().then(() => __importStar(require('../models/Resource')))).default;
