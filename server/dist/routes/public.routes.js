...
router.get('/teachers', async (_req, res) => {
    try {
        const teachers = await Promise.resolve().then(() => __importStar(require('../models/User'))).then(m => m.default.find({ role: 'teacher', approved: true })
...
router.get('/resources', async (req, res) => {
    try {
        ...
        // @ts-ignore
        const resources = await Promise.resolve().then(() => __importStar(require('../models/Resource'))).then(m => m.default.find(filter).populate('subjectId', 'title'));
...
router.get('/quizzes/:id', async (req, res) => {
    try {
        const quiz = await Quiz_1.default.findById(req.params.id).populate('subjectId', 'title');
        if (!quiz)
...
router.get('/admin-contact', async (_req, res) => {
    try {
        const admin = await Promise.resolve().then(() => __importStar(require('../models/User'))).then(m => m.default.findOne({ role: 'admin' }).select('_id name email'));
        if (!admin)
...
router.post('/attempt-quiz', auth_1.requireAuth, async (req, res) => {
    ...
    if (quiz.isPremium && req.user?.role === 'student') {
        const user = await Promise.resolve().then(() => __importStar(require('../models/User'))).then(m => m.default.findById(req.user.id));
