...
router.post('/attempt-quiz', requireAuth, async (req: AuthedRequest, res) => {
  ...
  if (!quizId || !Array.isArray(answers)) return res.status(400).json({ error: 'Missing fields' })
  ...
  if (quiz.isPremium && req.user?.role === 'student') {
    ...
    if (!active) return res.status(402).json({ error: 'Premium subscription required' })
  }
