...
const requireApprovedTeacher = async (req: AuthedRequest, res: any, next: any) => {
  ...
  if (!teacher.approved) return res.status(403).json({ error: 'Teacher not approved yet' })
    ; (req as any).teacherDoc = teacher
  next()
}

// GET /api/teacher/view-assigned-subjects
router.get('/view-assigned-subjects', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.post('/create-class', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  ...
  const teacher = (req as any).teacherDoc
  const allowed = teacher.assignedSubjects.some((id: any) => id.toString() === subjectId)
...
router.post('/create-quiz', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.put('/update-class/:classId', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.post('/create-resource', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.delete('/resource/:id', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const { id } = req.params
  const teacherId = req.user!.id

  const resource = await Resource.findOne({ _id: id, teacherId })
  if (!resource) return res.status(404).json({ error: 'Resource not found or unauthorized' })
...
router.get('/dashboard', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const teacherId = req.user!.id
...
router.get('/messages', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const teacher = (req as any).teacherDoc
  // Find messages where recipientId matches teacher's ID
  const messages = await import('../models/Message').then(m =>
    m.default.find({ recipientId: teacher._id })
      .sort({ createdAt: -1 })
...
router.get('/conversations', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const teacherId = req.user!.id
...
router.get('/messages/:userId', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const teacherId = req.user!.id
...
router.post('/settings', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.post('/availability', requireApprovedTeacher, async (req: AuthedRequest, res) => {
...
router.get('/availability', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const teacher = await User.findById(req.user!.id).select('availability')
  res.json(teacher?.availability || [])
})

// GET /api/teacher/bookings
router.get('/bookings', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const bookings = await import('../models/Booking').then(m =>
    m.default.find({ teacherId: req.user!.id })
...
router.post('/cancel-booking', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const { bookingId } = req.body
  await import('../models/Booking').then(m =>
    m.default.findByIdAndUpdate(bookingId, { status: 'cancelled' })
  )
  res.json({ success: true })
})

// PUT /api/teacher/booking/:bookingId
router.put('/booking/:bookingId', requireApprovedTeacher, async (req: AuthedRequest, res) => {
  const { bookingId } = req.params
  const { meetingLink } = req.body
  await import('../models/Booking').then(m =>
    m.default.findByIdAndUpdate(bookingId, { meetingLink })
  )
  res.json({ success: true })
})
